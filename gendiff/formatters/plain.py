...
# def format_node_recursively(diff_node: dict, path="") -> map | str | list:
#     """
#     Formats node and all its children
#     :param diff_node: node from difference generated by gendiff/gendiff.py
#     :param path: path to the node, no need to pass this value
#     :return: returns str, map object or a list (pf)
#     """
#     name = gendiff.get_name(diff_node)
#     status = gendiff.get_status(diff_node)
#     curr_path = path + "." + name if len(path) > 0 else name
#     if not gendiff.is_leaf(diff_node):
#         children = gendiff.get_children(diff_node)
#         return map(lambda node: format_node_recursively(node, curr_path), children)
#     elif status != "both":
#         return generate_diff_representation(diff_node, curr_path)
#     else:
#         # TODO CHANGE TO NONE??
#         return []
...
def plain(diff: list) -> str:
    ...
    # empty_lists_removed = filter(
    #     lambda el: type(el) != list, flatten(diff_formatted)
    # )
